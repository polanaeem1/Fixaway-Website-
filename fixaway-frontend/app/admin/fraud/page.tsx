  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        {active > 0 && (
          <span className="flex items-center gap-2 bg-error/10 text-error border border-error/20 px-4 py-2 rounded-xl text-sm font-bold self-start sm:self-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          <div key={s.label} className="bg-white rounded-2xl p-5 border border-outline-variant/20 shadow-sm flex flex-col justify-between">
            <div className={`w-10 h-10 rounded-xl ${s.color} flex items-center justify-center mb-3 flex-shrink-0`}>
            ...
            <div key={alert.id} className={`bg-white rounded-2xl border shadow-sm overflow-hidden ${alert.risk === 'CRITICAL' ? 'border-red-300' : 'border-outline-variant/20'}`}>
              <div className={`p-5 ${alert.risk === 'CRITICAL' ? 'bg-red-50' : ''}`}>
                  ...
                  <div className="flex items-start gap-4 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-1 flex-wrap">
                        ...
                        <span className="font-bold text-on-surface text-base break-words">{alert.type}</span>
                        <span className="text-xs text-outline sm:ml-auto">{timeAgo(alert.createdAt)}</span>
                      </div>
                      <p className="text-sm text-on-surface-variant mb-1 font-semibold">{alert.entity}</p>
                      <p className="text-sm text-on-surface-variant leading-relaxed">{alert.description}</p>
                    </div>
                  </div>
                  <div className="flex flex-row sm:flex-row md:flex-row gap-2 w-full md:w-auto md:flex-shrink-0">
                    <button 
                      onClick={() => openActionModal(alert)}
                      className="flex-1 md:flex-initial px-4 py-2 bg-error text-white rounded-xl text-sm font-bold hover:bg-error/90 transition-all active:scale-95 flex items-center justify-center gap-1.5 shadow-sm"
                    >
                    ...
                    <button 
                      ...
                      className="flex-1 md:flex-initial px-4 py-2 border border-outline-variant text-on-surface-variant rounded-xl text-sm font-semibold hover:bg-surface-container-low transition-colors text-center"
                    >
                      Dismiss
                    </button>
                  </div>
                ...
                <div className="flex items-center gap-3 mt-4 pt-4 border-t border-outline-variant/20">
                  <span className="text-xs text-outline">Alert ID: <span className="font-mono font-bold text-on-surface-variant break-all">{alert.id}</span></span>
                </div>
              </div>
            </div>
          ))}
      ...
      {actionModalOpen && selectedAlert && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm overflow-y-auto">
          <div className="bg-surface border border-outline-variant/30 rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-5 my-8">
            <div className="flex items-center gap-3 text-error">
              <span className="material-symbols-outlined text-3xl flex-shrink-0">gavel</span>
            ...
            <div className="bg-surface-container-low p-4 rounded-2xl border border-outline-variant/20 space-y-2">
              ...
              <p className="text-xs text-on-surface-variant font-mono break-all">{selectedAlert.id}</p>
              <p className="text-sm text-on-surface-variant italic">&ldquo;{selectedAlert.description}&rdquo;</p>
            </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  {selectedAlert.order.customer && (
                    ...
                    >
                      ...
                      <span className="font-bold text-sm text-center truncate w-full">{selectedAlert.order.customer.name}</span>
                  ...
                  {selectedAlert.order.technician && (
                    ...
                    >
                      ...
                      <span className="font-bold text-sm text-center truncate w-full">{selectedAlert.order.technician.name}</span>
